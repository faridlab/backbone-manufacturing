...
2. **Boilerplate is generic, so make it generic once.** Standard CRUD is not written per entity;
   ...
   A module's `BomService` is a **type alias**, not an `impl`. (See
...
Non-goals are as important as goals — they are why a module stays small and sharply bounded.
...
- **It does not own cross-domain logic.** One module = one bounded context. It never reaches into
  another module's schema or entities. User identity is *referenced* from the `sapiens` module by
  logical foreign key; accounts, warehouses, and catalog items are logical FKs to accounting,
  inventory, and catalog — never copied in. (See [The manufacturing domain](09-manufacturing-domain.md)
  for how this module posts to accounting and drives inventory without a Cargo dependency on either.)
- **It does not ship every layer up front.** A module carries Domain, Application,
  Infrastructure/persistence, Presentation/http, routes, and seeders. Event sourcing, CQRS, cache,
  gRPC, GraphQL, state machines, workflows — these are added when a real requirement demands them,
  not as speculative scaffolding. Manufacturing, for instance, adds a hand-authored write service
  and domain events only because its WIP/GL seam needs them.
