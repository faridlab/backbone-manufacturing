...
The versions below are what this module (crate **v0.1.3**) pins; where behavior is version-specific,
...
| Boundary errors | **`anyhow` 1.0** | Right tool at the *composition* boundary (`ManufacturingModuleBuilder::build` returns `anyhow::Result`) where a typed enum adds no value | `thiserror` everywhere (ceremony with no payoff at the boundary) |
...
| IDs / time / money | **`uuid` v4 + v5**, **`chrono`**, **`rust_decimal`** | UUID primary keys avoid enumeration and merge cleanly across modules; **v5** derives stable per-post `source_id`s for GL idempotency (`manufacturing_write_service.rs`); `chrono` for audit timestamps and posting dates; `rust_decimal` carries every cost/quantity (IDR, 2dp, half-away-from-zero) — the whole domain is decimal money | integer PKs (leak ordinality, collide across modules), `f64` money (rounding bugs) |
| Async traits | **`async-trait` 0.1** | The hand-authored seam traits (`InventoryPort`, `GlPostSink`, `ManufacturingEventSink`) are `async fn` in a trait — `async-trait` makes them object-safe so a composing service can supply any adapter | inherent `async fn` in traits (not yet object-safe for `dyn` dispatch on this toolchain) |
...
Four crates carry the leverage. In this module they are **git dependencies** on the public
...
| Crate | Gives the module | Seen in this module as |
|-------|------------------|------------------------|
| **`backbone-core`** | `GenericCrudService`, `BackboneCrudHandler`, `PersistentEntity`, `FromCreateDto` / `ApplyUpdateDto`, `ServiceError` / `ServiceResult` | the eight service type aliases (`BomService`, `WorkOrderService`, …), the handlers, DTO conversions, `service/error.rs` |
| **`backbone-orm`** | `GenericCrudRepository`, `SoftDelete`, `EntityRepoMeta`, pagination types | the eight repository newtypes, each entity's `EntityRepoMeta` impl |
