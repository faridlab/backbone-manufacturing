...
- **Conventional commits.** `type(scope): summary` — e.g. `feat(work-order): add short-close verb`,
  `fix(bom): correct operating-cost roll-up`, `docs(handbook): add manufacturing-domain page`. Types
  drive versioning: `fix:` → patch, `feat:` → minor, `feat!:` / `BREAKING CHANGE:` → major.
...
```
feat(work-order): reject receive before release

Business rule BR-5 (docs/BRD.md): a Work Order has no WIP to receive
until it is in_process. Enforced in manufacturing_write_service.rs so
...
- Integration and seam tests are database-backed: spin up Postgres, run the module migrations, build
  `ManufacturingModule::builder().with_database(pool).build()?`, and exercise `all_crud_routes()`
  (or `routes::get_routes(&module)`) via `axum::http::Request` + `tower::ServiceExt`. The domain
  verbs are tested directly against `ManufacturingWriteService` with in-test `InventoryPort` /
  `GlPostSink` adapters (see `tests/plant_to_produce_seam.rs`).
- Behavior tests and BDD features live under `tests/features/**`, which is a `user_owned` path — the
  ...
  with a `.feature` and a golden-case test; keep the three in step (the
  [golden cases](../business-flows/golden-cases.md) mirror the numbers one-to-one).
