...
- A repository is a **thin newtype** over `backbone_orm::GenericCrudRepository<Entity, SoftDelete>`,
  exposing all standard methods via `Deref` (as this module's `BomRepository` does):
  ```rust
  pub struct BomRepository(GenericCrudRepository<Bom, SoftDelete>);
  ```
- A service is a **type alias**, not an `impl`:
  ```rust
  pub type BomService =
      GenericCrudService<Bom, CreateBomDto, UpdateBomDto, BomRepository>;
  ```
- The HTTP surface is one call to `BackboneCrudHandler::routes(service, "/boms")`, which
