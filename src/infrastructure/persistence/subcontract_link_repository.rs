...
//! subcontract receipt path's IDEMPOTENCY BACKSTOP: one hidden work order per purchase order
//! (org-scoped under the composed fence, ADR-0029). A replayed receipt event finds the existing
//! link and returns the work order it minted — no second MO, no second cost leg.
...
impl SubcontractLinkRepository {
    ...
    /// break the replay backstop. The caller relays the ambient org scope on it
    /// (`relay_ambient_scope`) — don't re-bind (ADR-0029). Returns the raw `sqlx::Error` so a
    /// concurrent mint surfaces as a unique violation the caller can re-read.
    pub async fn insert_link(
        ...
        sqlx::query(
            r#"INSERT INTO manufacturing.subcontract_mo_links
                 (id, purchase_order_id, work_order_id)
               VALUES ($1,$2,$3)"#,
        )
        .bind(l.id).bind(l.purchase_order_id).bind(l.work_order_id)
        .execute(conn)
        .await?;
        Ok(())
    }
    ...
    /// stop (`find_by_purchase_order`). ID-only (ADR-0029): the read rides the
    /// request-dedicated connection; the decorator's re-declared (org unit, purchase order)
    /// unique keeps one link per order per unit.
