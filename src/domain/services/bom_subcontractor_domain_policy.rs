//! Domain policy for BomSubcontractor aggregate
//!
...
/// Domain policy for BomSubcontractor — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<BomSubcontractor>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type BomSubcontractorDomainPolicy = PermitAllPolicy<BomSubcontractor>;
