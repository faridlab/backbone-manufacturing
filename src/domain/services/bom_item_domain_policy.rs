//! Domain policy for BomItem aggregate
//!
...
/// Domain policy for BomItem — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<BomItem>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type BomItemDomainPolicy = PermitAllPolicy<BomItem>;
