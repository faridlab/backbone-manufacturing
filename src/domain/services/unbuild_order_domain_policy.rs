//! Domain policy for UnbuildOrder aggregate
//!
...
/// Domain policy for UnbuildOrder — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<UnbuildOrder>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type UnbuildOrderDomainPolicy = PermitAllPolicy<UnbuildOrder>;
