//! Domain policy for BomByproduct aggregate
//!
...
/// Domain policy for BomByproduct — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<BomByproduct>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type BomByproductDomainPolicy = PermitAllPolicy<BomByproduct>;
